        ...
        try:
            ...
            for page_num, page_data in enumerate(pages, 1):
                ...
                if missing:
                    ...
                    failed_count += 1
                    results.append({"page_name": page_data.page_name, "success": False, "error": f"Missing fields: {missing}"})
                    continue

                # Create page
                try:
                    # Navigate back to dashboard before processing each page
                    ...
                    success_count += 1
                    results.append({"page_name": page_data.page_name, "success": True, "error": None})

                except Exception as e:
                    logger.error(f"FAILED: Page {page_num} - {page_data.page_name}: {e}", exc_info=True)
                    failed_count += 1
                    results.append({"page_name": page_data.page_name, "success": False, "error": str(e)})

                    if config.screenshot_on_error:
                        automation.take_screenshot(f"error_{page_data.page_name}.png")
    ...
    for file_result in file_results:
        ...
        if file_result['details']:
            for page in file_result['details']:
                ...
                if not page['success']:
                    logger.info(f"      Error: {page.get('error', 'Unknown error')}")
